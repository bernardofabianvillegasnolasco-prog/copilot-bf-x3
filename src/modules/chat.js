...
async function callGroq(key, prompt) {
  const models = ["llama-3.3-70b-versatile", "llama3-8b-8192", "openai/gpt-oss-20b"];
  for(const model of models){
    try {
      ...
      if(data.choices?.[0]?.message?.content) return `[${model}] ${data.choices[0].message.content}`;
      if(data.error && !data.error.message.includes("does not exist")) return `Error: ${data.error.message}`;
    } catch(e){ continue; }
  }
